---Shared helpers for resource wrapper modules. Require this directly.
---
---Context note: I'm fully aware that Flux exposes actual "modern" way of using SOME resources types (textures, images, ...)
...
M.ENUM_RES_TYPE = { tex = 1, img = 2, ani = 3, bgm = 4, snd = 5, psi = 6, fnt = 7, ttf = 8, fx = 9, model = 10, video = 11 }
---@alias ResourceTypes
---| "tex"
---| "img"
---| "ani"
...
---@return string
function M.get_typed_name(typename, file_path)
    local t = M.ENUM_RES_TYPE[typename]
...
function M.set_default_sampler_state(state)
    M.default_sampler_state = state
end
...
function lstg.RemoveResource(pool, restype, resname)
    local t = M.ENUM_RES_TYPE[restype]
    if t == nil then
...
function M.set_active_pool(pool_name)
...
function M.transfer(from, t, name, to)
